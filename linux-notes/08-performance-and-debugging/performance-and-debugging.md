...
- [1. Concept (Senior-Level Understanding)](#1-concept-senior-level-understanding)
  - [The Observability Philosophy: From Monitoring to Understanding](#the-observability-philosophy-from-monitoring-to-understanding)
  - [The USE Method (Brendan Gregg)](#the-use-method-brendan-gregg)
  - [The RED Method (Tom Wilkie)](#the-red-method-tom-wilkie)
  - [Observability Architecture Overview](#observability-architecture-overview)
- [2. Internal Working (Kernel-Level Deep Dive)](#2-internal-working-kernel-level-deep-dive)
  - [Linux Observability Tool Landscape by Subsystem](#linux-observability-tool-landscape-by-subsystem)
  - [eBPF Architecture: From Userspace to Kernel Hooks](#ebpf-architecture-from-userspace-to-kernel-hooks)
  - [BCC vs. bpftrace: Choosing the Right Frontend](#bcc-vs-bpftrace-choosing-the-right-frontend)
  - [strace Internals: The ptrace Tax](#strace-internals-the-ptrace-tax)
  - [perf Subsystem Architecture](#perf-subsystem-architecture)
  - [Pressure Stall Information (PSI)](#pressure-stall-information-psi)
  - [ftrace and trace-cmd: In-Kernel Tracing](#ftrace-and-trace-cmd-in-kernel-tracing)
  - [kdump and crash: Post-Mortem Kernel Analysis](#kdump-and-crash-post-mortem-kernel-analysis)
- [3. Commands (Master Reference)](#3-commands-master-reference)
  - [vmstat -- Virtual Memory Statistics](#vmstat----virtual-memory-statistics)
  - [iostat -- I/O Statistics](#iostat----io-statistics)
  - [mpstat -- Multiprocessor Statistics](#mpstat----multiprocessor-statistics)
  - [pidstat -- Per-Process Statistics](#pidstat----per-process-statistics)
  - [sar -- System Activity Reporter (Historical)](#sar----system-activity-reporter-historical)
  - [strace -- System Call Tracing](#strace----system-call-tracing)
  - [perf -- Performance Profiler](#perf----performance-profiler)
  - [bpftrace -- eBPF Dynamic Tracing](#bpftrace----ebpf-dynamic-tracing)
  - [BCC Tools (Essential Kit)](#bcc-tools-essential-kit)
  - [ss and Network Diagnostics](#ss-and-network-diagnostics)
  - [Pressure Stall Information](#pressure-stall-information)
  - [/proc Filesystem -- Key Performance Files](#proc-filesystem----key-performance-files)
- [4. Debugging (The Master Section)](#4-debugging-the-master-section)
  - [The Master Debugging Flowchart: "My Server Is Slow"](#the-master-debugging-flowchart-my-server-is-slow)
  - [Debugging Methodology: The 60-Second Checklist](#debugging-methodology-the-60-second-checklist)
  - [Debugging Specific Scenarios](#debugging-specific-scenarios)
- [5. Real-World Incidents (5 Production Scenarios)](#5-real-world-incidents-5-production-scenarios)
  - [Incident 1: High CPU from Short-Lived Fork Bombs Invisible to top](#incident-1-high-cpu-from-short-lived-fork-bombs-invisible-to-top)
  - [Incident 2: Memory Leak Diagnosed with eBPF on a Kubernetes Node](#incident-2-memory-leak-diagnosed-with-ebpf-on-a-kubernetes-node)
  - [Incident 3: Disk Latency from Wrong I/O Scheduler](#incident-3-disk-latency-from-wrong-io-scheduler)
  - [Incident 4: Network Microbursts Causing TCP Retransmissions](#incident-4-network-microbursts-causing-tcp-retransmissions)
  - [Incident 5: Kernel Soft Lockup from Spinlock Contention](#incident-5-kernel-soft-lockup-from-spinlock-contention)
- [6. Interview Questions (15+)](#6-interview-questions-15)
  - [Q1: Walk me through how you would debug a slow server. What is your methodology?](#q1-walk-me-through-how-you-would-debug-a-slow-server-what-is-your-methodology)
  - [Q2: Explain the USE method. Apply it to diagnose CPU saturation.](#q2-explain-the-use-method-apply-it-to-diagnose-cpu-saturation)
  - [Q3: What is the difference between perf and strace? When would you use each?](#q3-what-is-the-difference-between-perf-and-strace-when-would-you-use-each)
  - [Q4: Explain eBPF architecture. Why is it safe to run in the production kernel?](#q4-explain-ebpf-architecture-why-is-it-safe-to-run-in-the-production-kernel)
  ...
  - [Q8: How does strace work internally? Why is its overhead so high?](#q8-how-does-strace-work-internally-why-is-its-overhead-so-high)
  - [Q9: Explain the differences between vmstat, mpstat, pidstat, and sar. When do you use each?](#q9-explain-the-differences-between-vmstat-mpstat-pidstat-and-sar-when-do-you-use-each)
  ...
  - [Q11: What is the difference between load average and CPU utilization? A server has load average 64 but only 10% CPU utilization -- what is happening?](#q11-what-is-the-difference-between-load-average-and-cpu-utilization-a-server-has-load-average-64-but-only-10-cpu-utilization----what-is-happening)
  - [Q12: How would you use eBPF/bpftrace to trace why a specific file is being written to?](#q12-how-would-you-use-ebpfbpftrace-to-trace-why-a-specific-file-is-being-written-to)
  - [Q13: Explain what happens when the kernel OOM killer activates. How do you control it?](#q13-explain-what-happens-when-the-kernel-oom-killer-activates-how-do-you-control-it)
  - [Q14: How would you debug a process stuck in D (uninterruptible sleep) state?](#q14-how-would-you-debug-a-process-stuck-in-d-uninterruptible-sleep-state)
  - [Q15: What is ftrace and when would you use it instead of perf or eBPF?](#q15-what-is-ftrace-and-when-would-you-use-it-instead-of-perf-or-ebpf)
  - [Q16: A Java application is consuming 8 GiB RSS but free -m shows 2 GiB available and the system is not swapping. Should you be concerned?](#q16-a-java-application-is-consuming-8-gib-rss-but-free--m-shows-2-gib-available-and-the-system-is-not-swapping-should-you-be-concerned)
- [7. Common Pitfalls](#7-common-pitfalls)
  - [Pitfall 1: Alerting on MemFree Instead of MemAvailable](#pitfall-1-alerting-on-memfree-instead-of-memavailable)
  - [Pitfall 2: Trusting %util from iostat on NVMe/RAID](#pitfall-2-trusting-util-from-iostat-on-nvmeraid)
  - [Pitfall 3: Ignoring the First Line of vmstat](#pitfall-3-ignoring-the-first-line-of-vmstat)
  - [Pitfall 4: Using strace in Production](#pitfall-4-using-strace-in-production)
  - [Pitfall 5: Assuming Load Average = CPU Load](#pitfall-5-assuming-load-average-cpu-load)
  - [Pitfall 6: Not Accounting for Stolen Time in VMs](#pitfall-6-not-accounting-for-stolen-time-in-vms)
  - [Pitfall 7: Using 5-Minute Averages to Detect Microbursts](#pitfall-7-using-5-minute-averages-to-detect-microbursts)
  - [Pitfall 8: Forgetting CFQ I/O Scheduler on SSDs](#pitfall-8-forgetting-cfq-io-scheduler-on-ssds)
  - [Pitfall 9: Not Sizing Conntrack Hash Tables](#pitfall-9-not-sizing-conntrack-hash-tables)
  - [Pitfall 10: Treating All Retransmissions as Network Problems](#pitfall-10-treating-all-retransmissions-as-network-problems)
- [8. Pro Tips](#8-pro-tips)
  - [Pro Tip 1: PSI Triggers for Proactive Response](#pro-tip-1-psi-triggers-for-proactive-response)
  - [Pro Tip 2: The Off-CPU Flame Graph](#pro-tip-2-the-off-cpu-flame-graph)
  - [Pro Tip 3: perf stat for IPC (Instructions Per Cycle)](#pro-tip-3-perf-stat-for-ipc-instructions-per-cycle)
  - [Pro Tip 4: funccount for Rapid Kernel Function Frequency](#pro-tip-4-funccount-for-rapid-kernel-function-frequency)
  - [Pro Tip 5: Keep a Production Baseline](#pro-tip-5-keep-a-production-baseline)
  - [Pro Tip 6: kdump Is Your Safety Net for Kernel Panics](#pro-tip-6-kdump-is-your-safety-net-for-kernel-panics)
  - [Pro Tip 7: Combine USE + RED for Complete Coverage](#pro-tip-7-combine-use-red-for-complete-coverage)
- [9. Cheatsheet (Quick Reference)](#9-cheatsheet-quick-reference)
  ...
  - [PSI Quick Reference](#psi-quick-reference)
  - [Essential BCC/bpftrace One-Liners](#essential-bccbpftrace-one-liners)
  - [Key /proc Files for Performance](#key-proc-files-for-performance)
  - [Diagnostic Decision Tree (Quick)](#diagnostic-decision-tree-quick)
